...
let choixJourRadioButtons = document.querySelectorAll(
  'input[name="choixJour"]'
);
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  function updatePrices() {
    ...
    [pass1jour, pass2jours, pass3jours].forEach(function (pass, index) {
      ...
      pass.nextElementSibling.textContent = `Pass ${
        index + 1
      } jour(s) : ${price}€`;
    });
  ...
  function updatePrixTotal() {
    ...
    let selectedPass = [pass1jour, pass2jours, pass3jours].find(
      (pass) => pass.checked
    );
...
function checkJours() {
  ...
  if (pass1jourChecked) {
    joursRadioButtons = document.querySelectorAll(
      '#pass1jourDate input[name="choixJour"]'
    );
  } else if (pass2joursChecked) {
    joursRadioButtons = document.querySelectorAll(
      '#pass2joursDate input[name="choixJour"]'
    );
  } else {
    alert("n'oubliez pas de choisir au moins une formule");
    return false;
  ...
  var atLeastOneDaySelected = Array.from(joursRadioButtons).some(function (
    radio
  ) {
...
function suivant(section) {
  ...
  if (currentSection.id === "reservation") {
    ...
    if (!validReservation && !formuleValid) {
      alert(
        "Entrez un nombre de réservation, et n'oubliez pas de choisir au moins une formule"
      );
      return;
